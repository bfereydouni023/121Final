...
export function createGoal(
  ...
  m.mesh = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0x00ff88, opacity: 0.9, transparent: true })
  );
  ...
  try {
    ...
    rb.addCollider(RAPIER.ColliderDesc.cuboid(size.x / 2, size.y / 2, size.z / 2), true);
  } catch {
  ...
  script.onPhysicsUpdate = () => {
    ...
    const ballPos = (ballObj as THREE.Object3D).getWorldPosition(new THREE.Vector3());
    if (box.containsPoint(ballPos)) {
      triggered = true;
      // dispatch a generic event so UI / main can listen and transition to a victory screen
      window.dispatchEvent(new CustomEvent('game:victory', { detail: { goalId: goal.id } }));
...
export function createOverlay(
  ...
  script.onPhysicsUpdate = () => {
    ...
    if (box.containsPoint(ballPos)) {
      triggered = true;
      // dispatch a generic event so UI / main can listen and transition to a victory screen
      window.dispatchEvent(new CustomEvent('game:victory', { detail: { goalId: overlay.id } }));
      // optional: visual feedback
      (m.mesh.material as THREE.Material).opacity = 0.5;
