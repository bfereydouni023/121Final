...
export function createDoor(
  ...
  rb.addCollider(RAPIER.ColliderDesc.cuboid(size.x / 2, size.y / 2, size.z / 2), false);
  ...
  function openDoor() {
    ...
    try {
      if (typeof onOpen === "function") onOpen(go as any);
    } catch (err) {}
  ...
  script.onCollisionEnter = (other) => {
    ...
    const otherUD = (other as any)?.getComponent
      ? (other as any).getComponent
      : undefined;
    // simpler check: look at mesh/userData via traversing components (best-effort)
    const playerGO = getObjectByID("player");
    const isPlayer = other === playerGO || (other as any)?.id === "player";
    if (!isPlayer) {
      // also allow collisions from objects tagged as player in their mesh userData
      try {
        const mesh = (other as any)?.getComponent?.(MeshComponent) as MeshComponent | null;
        if (mesh && mesh.mesh && (mesh.mesh.userData?.type === "player")) {
          // treat as player
        }
      } catch {}
    }
